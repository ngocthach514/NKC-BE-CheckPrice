...
class DofZoneDofHandler extends BaseHandler {
  ...
  async search(keyword) {
    ...
    try {
      const url = this.site.api_url.replace(
        "{keyword}",
      ...
      if (!product) {
        console.log(
          `❌ Kết quả cho site ${
            this.site.name
          }: KHÔNG TÌM THẤY với từ khóa '${keyword}' trong ${
            (json?.results || []).length
          } kết quả`
        );
      ...
      const serial =
        product.code || product.sku || product.productModel || null;

      console.log(
        `✅ Kết quả cho site ${this.site.name}: ${name} | Giá: ${price}`
      );
      console.log(`🔚 Kết thúc site ${this.site.name}`);
      console.log("=".repeat(100));
